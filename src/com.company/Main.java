...
/**
 * @author Бельденков Даниил
 */

public class Main {
    public static void main(String[] args) {
        String inputString;
        Pattern p = Pattern.compile("([+]|-|[*]|/|^|%)"); //Метод compile позволяет установить шаблон для регулярного выражения.
        Matcher m;  //Matchet это поисковик регулярных выражений
        double result = 0; //Выводим начальные инструкции для пользователя
        System.out.println("Введите операцию типа(a + n): ");
        try (Scanner scanner = new Scanner(System.in)) { //обработка исключени
            while (true) { //Цикл while выполняет действие пока истинно условие
                if (scanner.hasNextLine()) {
                    inputString = scanner.nextLine(); //nextLine считывает всю введённую строку
        ...
        } catch (Exception e) { //продожение обработки исключений
    ...
    private static double calculate(String[] split, double previousResult) throws Exception {
        ...
        double number1;
        double number2;
        if (split.length == 3) { //Метод parse используется для извлечения из строки числового значения необходимого типа
            number1 = Double.parseDouble(split[0]);
            operand = split[1];
            number2 = Double.parseDouble(split[2]);

        } else if (split.length == 2) { //В случае провала в части if в части else срабатывает новая конструкция if...else
